...
const LIVE_ROOMS = [
  { id:'music', title:'Music Lounge', people:14, desc:'Talk about songs, artists and live concerts.', color:'#7B5DFF', icon:'🎧', avatars:['#7B5DFF','#EC4899','#10b981','#F59E0B'] },
  { id:'travel', title:'Travel Talk', people:9, desc:'Share travel stories, tips and experiences.', color:'#0EA5E9', icon:'✈️', avatars:['#06B6D4','#8B5CF6','#F59E0B','#EF4444'] },
  { id:'tech', title:'Tech Hub', people:12, desc:'Discuss tech, AI, coding and more.', color:'#EC4899', icon:'🖥️', avatars:['#6366F1','#10B981','#F59E0B','#EC4899'] },
];

const QUICK_GAMES = [
  { id:'trivia', title:'Metro Trivia', playing:234, desc:'Test your knowledge and win points.', icon:'🏆', color:'#7B5DFF' },
  { id:'would', title:'Would You Rather?', playing:189, desc:'Fun choices, crazy questions!', icon:'💬', color:'#EC4899' },
    ...
    <div className="animate-fade-in" style={{ paddingBottom: 86 }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12 }}>
        <div style={{ display:'flex', alignItems:'center', gap:8, fontSize:18, fontWeight:900, letterSpacing:-0.3 }}>
          <span style={{ color:'#7B5DFF' }}>Co</span><span style={{ color:'#EC4899' }}>Ride</span>
      ...
      <div style={{ background:'var(--bg-card)', border:'1px solid var(--border-card)', borderRadius:'var(--radius-xl)', padding:12, display:'flex', alignItems:'center', gap:12, marginBottom:14 }}>
        <div style={{ width:40,height:40, borderRadius:'50%', background:'linear-gradient(135deg, #7B5DFF, #4F46E5)', display:'flex', alignItems:'center', justifyContent:'center', color:'white' }}>
          <MapPin size={18}/>
        </div>
        <div style={{ flex:1 }}>
          <div style={{ fontSize:11, color:'var(--text-muted)', fontWeight:700 }}>You’re on</div>
          <div style={{ fontSize:14, fontWeight:800, color:'white' }}>{line}</div>
          <div style={{ fontSize:11, color:'var(--text-muted)' }}>{from} → {to}</div>
        </div>
        <div style={{ textAlign:'right' }}>
          <div style={{ fontSize:11, color:'#C4B5FF', fontWeight:700 }}>Next Stop</div>
          <div style={{ fontSize:13, fontWeight:800, color:'white' }}>{nextStop}</div>
          <button style={{ marginTop:4, width:32,height:32, borderRadius:'50%', background:'var(--bg-surface)', border:'1px solid var(--border-subtle)', display:'flex', alignItems:'center', justifyContent:'center', color:'var(--text-muted)' }}>⇄</button>
        </div>
      </div>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
        ...
        <button onClick={onSeeAllPeople} style={{ background:'none', border:'none', color:'var(--accent-violet)', fontSize:12, fontWeight:700 }}>See all ›</button>
      </div>
      ...
      <div style={{ display:'flex', gap:10, overflowX:'auto', paddingBottom:8, marginBottom:14 }}>
        {people.map((p:any, idx:number)=>{
          ...
          const ring = ['#7B5DFF','#10b981','#F59E0B','#EC4899','#6366F1'][idx%5];
          return (
            <div key={profile.id} onClick={()=> onProfile(profile)} style={{ flex:'0 0 96px', background:'var(--bg-card)', border:'1px solid var(--border-card)', borderRadius:'var(--radius-lg)', padding:'10px 8px', textAlign:'center', cursor:'pointer' }}>
              ...
              <div style={{ fontSize:10, color:'var(--text-muted)' }}>{dist} km away</div>
              <div style={{ marginTop:6, padding:'4px 6px', borderRadius:999, background:m.bg, border:'1px solid rgba(255,255,255,0.06)', fontSize:10, fontWeight:700, color:'var(--text-secondary)', display:'inline-flex', alignItems:'center', gap:4 }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
        <h3 style={{ fontSize:14, fontWeight:800, display:'flex', alignItems:'center', gap:6 }}><span style={{ color:'#EC4899' }}>♨</span> Live Rooms <span style={{ fontSize:11, color:'var(--text-muted)', fontWeight:500 }}>Join a room and start talking</span></h3>
        <button onClick={onSeeAllPeople} style={{ background:'none', border:'none', color:'var(--accent-violet)', fontSize:12, fontWeight:700 }}>View all ›</button>
      </div>
      <div style={{ display:'flex', gap:10, overflowX:'auto', paddingBottom:8, marginBottom:14 }}>
        {LIVE_ROOMS.map(r=>(
          <div key={r.id} style={{ flex:'0 0 180px', background:'var(--bg-card)', border:'1px solid var(--border-card)', borderRadius:'var(--radius-lg)', padding:12, display:'flex', flexDirection:'column', gap:8 }}>
            <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              ...
              <span style={{ fontSize:13, fontWeight:800, color:'white' }}>{r.title}</span>
            </div>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:0 }}>
              ...
              <span style={{ marginLeft:6, fontSize:10, padding:'2px 6px', borderRadius:999, background:'var(--bg-surface)', border:'1px solid var(--border-subtle)', color:'var(--text-muted)' }}>+{r.people -4}</span>
            </div>
            <div style={{ fontSize:11, color:'var(--text-muted)', lineHeight:1.3 }}>{r.desc}</div>
            <button onClick={()=> onJoinRoom(r.id, r.title)} style={{ marginTop:4, padding:'8px', borderRadius:'var(--radius-full)', background:'#7B5DFF', border:'none', color:'white', fontWeight:700, fontSize:12 }}>Join Room</button>
          </div>
        ))}
      </div>

      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
        <h3 style={{ fontSize:14, fontWeight:800, display:'flex', alignItems:'center', gap:6 }}><span style={{ color:'#0EA5E9' }}>🎮</span> Quick Games <span style={{ fontSize:11, color:'var(--text-muted)', fontWeight:500 }}>Play short games with people around you</span></h3>
        <button style={{ background:'none', border:'none', color:'var(--accent-violet)', fontSize:12, fontWeight:700 }}>All Games ›</button>
      </div>
      <div style={{ display:'flex', gap:10, overflowX:'auto', paddingBottom:8, marginBottom:14 }}>
        {QUICK_GAMES.map(g=>(
          <div key={g.id} style={{ flex:'0 0 150px', background:'var(--bg-card)', border:'1px solid var(--border-card)', borderRadius:'var(--radius-lg)', padding:12, display:'flex', flexDirection:'column', gap:8 }}>
            ...
            <div style={{ fontSize:12, fontWeight:800, color:'white' }}>{g.title}</div>
      ...
      <div style={{ display:'flex', gap:10, marginBottom:12 }}>
        <div style={{ flex:1, background:'var(--bg-card)', border:'1px solid var(--border-card)', borderRadius:'var(--radius-lg)', padding:12, display:'flex', alignItems:'center', gap:10 }}>
          ...
          <div>
            <div style={{ fontSize:16, fontWeight:900, color:'white' }}>342</div>
            <div style={{ fontSize:11, color:'var(--text-muted)' }}>people on this line</div>
          </div>
        </div>
        <div style={{ flex:1, background:'var(--bg-card)', border:'1px solid var(--border-card)', borderRadius:'var(--radius-lg)', padding:12, display:'flex', alignItems:'center', gap:10 }}>
          <div style={{ width:32,height:32, borderRadius:10, background:'rgba(123,93,255,0.14)', display:'flex', alignItems:'center', justifyContent:'center', color:'#7B5DFF' }}>🎁</div>
          <div>
            <div style={{ fontSize:12, fontWeight:800, color:'white' }}>Invite Friends</div>
      ...
      <div style={{ background:'var(--bg-card)', border:'1px solid var(--border-card)', borderRadius:'var(--radius-lg)', padding:12, display:'flex', gap:10, alignItems:'center' }}>
        <div style={{ width:36,height:36, borderRadius:'50%', background:'rgba(123,93,255,0.14)', display:'flex', alignItems:'center', justifyContent:'center', color:'#7B5DFF' }}><Shield size={18}/></div>
        <div style={{ flex:1 }}>
          <div style={{ fontSize:12, fontWeight:800, color:'white' }}>Safety First</div>
