...
export const OnboardingScreen: React.FC<Props> = ({ user, onComplete, onSkip }) => {
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:80, background:'var(--bg-base)', overflowY:'auto', padding:'16px 12px 24px' }}>
      {/* Progress header */}
      <div style={{ maxWidth:520, margin:'0 auto' }}>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:16 }}>
          ...
          <div style={{ flex:1, margin:'0 16px', height:4, borderRadius:999, background:'var(--bg-surface)', display:'flex', gap:4, alignItems:'center' }}>
            <div style={{ flex:1, height:4, borderRadius:999, background: step>=1 ? 'linear-gradient(90deg, #7B5DFF, #EC4899)' : 'var(--border-subtle)' }} />
            <div style={{ flex:1, height:4, borderRadius:999, background: step>=2 ? 'linear-gradient(90deg, #7B5DFF, #EC4899)' : 'var(--border-subtle)' }} />
        ...
        {step===1 ? (
          <div className="animate-fade-in">
            ...
            <div style={{ margin:'20px auto 0', width:'100%', maxWidth:320, height:180, borderRadius:'var(--radius-xl)', background:'linear-gradient(180deg, #1A1A2E 0%, #0A0A0F 100%)', border:'1px solid var(--border-card)', display:'flex', alignItems:'center', justifyContent:'center', position:'relative', overflow:'hidden' }}>
              <div style={{ position:'absolute', inset:0, opacity:0.12, background:'radial-gradient(circle at 50% 30%, #7B5DFF, transparent 60%)' }} />
              <div style={{ fontSize:48 }}>🚇</div>
              <div style={{ position:'absolute', bottom:12, left:12, right:12, height:2, background:'linear-gradient(90deg, #7B5DFF, #EC4899)', borderRadius:999, opacity:0.6 }} />
              <div style={{ position:'absolute', top:12, right:14, width:10,height:10, borderRadius:'50%', background:'#EC4899', boxShadow:'0 0 8px #EC4899' }} />
              <div style={{ position:'absolute', bottom:24, left:18, width:10,height:10, borderRadius:'50%', background:'#7B5DFF', border:'2px solid white' }} />
            </div>

            <div style={{ marginTop:16, display:'flex', flexDirection:'column', gap:10 }}>
              <label style={{ fontSize:11, color:'var(--text-muted)', fontWeight:700 }}>Select Metro Line
                <select value={line} onChange={e=> setLine(e.target.value)} style={{ marginTop:6, width:'100%', padding:'12px', borderRadius:'var(--radius-md)', background:'var(--bg-card)', border:'1px solid var(--border-card)', color:'white', fontSize:13 }}>
              ...
              <label style={{ fontSize:11, color:'var(--text-muted)', fontWeight:700 }}>From Station
                <select value={from} onChange={e=> setFrom(e.target.value)} style={{ marginTop:6, width:'100%', padding:'12px', borderRadius:'var(--radius-md)', background:'var(--bg-card)', border:'1px solid var(--border-card)', color:'white', fontSize:13 }}>
              ...
              <label style={{ fontSize:11, color:'var(--text-muted)', fontWeight:700 }}>To Station
                <select value={to} onChange={e=> setTo(e.target.value)} style={{ marginTop:6, width:'100%', padding:'12px', borderRadius:'var(--radius-md)', background:'var(--bg-card)', border:'1px solid var(--border-card)', color:'white', fontSize:13 }}>
              ...
              <button onClick={saveJourney} style={{ marginTop:8, padding:'14px', borderRadius:'var(--radius-full)', background:'linear-gradient(135deg, #7B5DFF, #EC4899)', border:'none', color:'white', fontWeight:800, fontSize:14, display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
            ...
            <div style={{ marginTop:20, background:'var(--bg-card)', border:'1px solid var(--border-card)', borderRadius:'var(--radius-lg)', padding:12, display:'flex', gap:10, alignItems:'center' }}>
              <div style={{ width:36,height:36, borderRadius:'50%', background:'rgba(123,93,255,0.14)', display:'flex', alignItems:'center', justifyContent:'center', color:'#7B5DFF' }}><ShieldCheck size={18}/></div>
              <div>
                <div style={{ fontSize:12, fontWeight:800, color:'white' }}>Your safety is our priority</div>
              ...
              {INTEREST_TAXONOMY.slice(0,12).map(t=> {
                ...
                return (
                  <button key={t.id} onClick={()=> toggleTag(t.id)} style={{
                    ...
                    {active && <span style={{ marginLeft:'auto', width:18,height:18, borderRadius:'50%', background:'#7B5DFF', color:'white', display:'flex', alignItems:'center', justifyContent:'center', fontSize:10 }}>✓</span>}
                  </button>
                );
              })}
            </div>
            <button onClick={finish} disabled={tags.length===0} style={{ marginTop:16, width:'100%', padding:'14px', borderRadius:'var(--radius-full)', background: tags.length? 'linear-gradient(135deg, #7B5DFF, #EC4899)' : 'var(--bg-surface)', border:'none', color:'white', fontWeight:800, fontSize:14, opacity: tags.length?1:0.5, display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
              Continue <ArrowRight size={16}/>
            </button>
            <p style={{ fontSize:10, color:'var(--text-muted)', textAlign:'center', marginTop:8 }}>{tags.length}/5 selected • You can change later in Profile</p>
          </div>
        )}
      </div>
    </div>
