...
export const BottomNav: React.FC<Props> = ({ active, onNavigate, unreadChats = 0 }) => {
  ...
  return (
    <nav className="bottom-nav" aria-label="Primary">
      ...
      <button className={`bottom-nav-item ${isPeopleActive ? 'active' : ''}`} onClick={() => onNavigate('people')}>
        <Users />
        <span>People</span>
      </button>
      ...
      <button className={`bottom-nav-item ${isChatsActive ? 'active' : ''}`} onClick={() => onNavigate('chats')} style={{ position:'relative' }}>
        <MessageCircle />
        <span>Chats</span>
        {unreadChats > 0 && (
          <span style={{ position:'absolute', top:2, right:18, minWidth:16, height:16, padding:'0 4px', borderRadius:999, background:'#EF4444', color:'white', fontSize:10, fontWeight:800, display:'flex', alignItems:'center', justifyContent:'center' }}>{unreadChats > 9 ? '9+' : unreadChats}</span>
        )}
      </button>
